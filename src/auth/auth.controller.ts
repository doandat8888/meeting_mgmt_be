...
export class AuthController {
    ...
    @Post('login')
    signIn(@Body() createUserDto: CreateUserDto, @Res({ passthrough: true }) response: Response) {
        return this.authService.signIn(createUserDto.email, createUserDto.password, response);
    }

    @Serialize(UserDto)
    @Post('register')
    signup(@Body(new ValidationPipe()) createUserDto: CreateUserDto) {
        return this.authService.signUp(createUserDto.email, createUserDto.password, createUserDto.fullName);
    }
